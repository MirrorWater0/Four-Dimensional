...
- Source: LookAtModifier3D.xml
- Inherits: SkeletonModifier3D
- Inheritance Chain: LookAtModifier3D -> SkeletonModifier3D -> Node3D -> Node -> Object
...
```
[methods]
get_interpolation_remaining() -> float [const]
is_interpolating() -> bool [const]
is_target_within_limitation() -> bool [const]
...
- is_interpolating() -> bool [const]
  Returns true if time-based interpolation is running. If true, it is equivalent to get_interpolation_remaining() returning 0.0. This is useful to determine whether a LookAtModifier3D can be removed safely.
...
- duration: float = 0.0 [set set_duration; get get_duration]
...
- origin_safe_margin: float = 0.1 [set set_origin_safe_margin; get get_origin_safe_margin]
...
- primary_damp_threshold: float [set set_primary_damp_threshold; get get_primary_damp_threshold]
...
- primary_limit_angle: float [set set_primary_limit_angle; get get_primary_limit_angle]
  The limit angle of the primary rotation when symmetry_limitation is true, in radians.

- primary_negative_damp_threshold: float [set set_primary_negative_damp_threshold; get get_primary_negative_damp_threshold]
  The threshold to start damping for primary_negative_limit_angle.

- primary_negative_limit_angle: float [set set_primary_negative_limit_angle; get get_primary_negative_limit_angle]
  The limit angle of negative side of the primary rotation when symmetry_limitation is false, in radians.

- primary_positive_damp_threshold: float [set set_primary_positive_damp_threshold; get get_primary_positive_damp_threshold]
  The threshold to start damping for primary_positive_limit_angle.

- primary_positive_limit_angle: float [set set_primary_positive_limit_angle; get get_primary_positive_limit_angle]
  The limit angle of positive side of the primary rotation when symmetry_limitation is false, in radians.

- primary_rotation_axis: int (Vector3.Axis) = 1 [set set_primary_rotation_axis; get get_primary_rotation_axis]
...
- relative: bool = false [set set_relative; get is_relative]
  The relative option. If true, the rotation is applied relative to the pose. If false, the rotation is applied relative to the rest. It means to replace the current pose with the LookAtModifier3D's result. **Note:** This option affects the base angle for use_angle_limitation unlike IterateIK3D's JointLimitation3D. Since the LookAtModifier3D relies strongly on Euler rotation, the axis that determines the limitation and the actual rotation are strongly tied together.

- secondary_damp_threshold: float [set set_secondary_damp_threshold; get get_secondary_damp_threshold]
  The threshold to start damping for secondary_limit_angle.

- secondary_limit_angle: float [set set_secondary_limit_angle; get get_secondary_limit_angle]
  The limit angle of the secondary rotation when symmetry_limitation is true, in radians.

- secondary_negative_damp_threshold: float [set set_secondary_negative_damp_threshold; get get_secondary_negative_damp_threshold]
  The threshold to start damping for secondary_negative_limit_angle.

- secondary_negative_limit_angle: float [set set_secondary_negative_limit_angle; get get_secondary_negative_limit_angle]
  The limit angle of negative side of the secondary rotation when symmetry_limitation is false, in radians.

- secondary_positive_damp_threshold: float [set set_secondary_positive_damp_threshold; get get_secondary_positive_damp_threshold]
  The threshold to start damping for secondary_positive_limit_angle.

- secondary_positive_limit_angle: float [set set_secondary_positive_limit_angle; get get_secondary_positive_limit_angle]
  The limit angle of positive side of the secondary rotation when symmetry_limitation is false, in radians.

- symmetry_limitation: bool [set set_symmetry_limitation; get is_limitation_symmetry]
...
- target_node: NodePath = NodePath("") [set set_target_node; get get_target_node]
...
- transition_type: int (Tween.TransitionType) = 0 [set set_transition_type; get get_transition_type]
  The transition type of the time-based interpolation. See also Tween.TransitionType.

- use_angle_limitation: bool = false [set set_use_angle_limitation; get is_using_angle_limitation]
...
- use_secondary_rotation: bool = true [set set_use_secondary_rotation; get is_using_secondary_rotation]
...
- ORIGIN_FROM_EXTERNAL_NODE = 2
  The global position of the Node3D specified in origin_external_node is used as origin. **Note:** Same as ORIGIN_FROM_SPECIFIC_BONE, when specifying a BoneAttachment3D with a child bone assigned, the rendered result and direction will not match.
