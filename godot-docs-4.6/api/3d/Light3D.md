...
- Inheritance Chain: Light3D -> VisualInstance3D -> Node3D -> Node -> Object
...
```
[methods]
get_correlated_color() -> Color [const]
get_param(param: int (Light3D.Param)) -> float [const]
set_param(param: int (Light3D.Param), value: float) -> void
...
- distance_fade_begin: float = 40.0 [set set_distance_fade_begin; get get_distance_fade_begin]
...
- distance_fade_enabled: bool = false [set set_enable_distance_fade; get is_distance_fade_enabled]
...
- distance_fade_length: float = 10.0 [set set_distance_fade_length; get get_distance_fade_length]
...
- distance_fade_shadow: float = 50.0 [set set_distance_fade_shadow; get get_distance_fade_shadow]
...
- light_angular_distance: float = 0.0 [set set_param; get get_param]
...
- light_bake_mode: int (Light3D.BakeMode) = 2 [set set_bake_mode; get get_bake_mode]
  The light's bake mode. This will affect the global illumination techniques that have an effect on the light's rendering. **Note:** Meshes' global illumination mode will also affect the global illumination rendering. See GeometryInstance3D.gi_mode.

- light_color: Color = Color(1, 1, 1, 1) [set set_color; get get_color]
...
- light_cull_mask: int = 4294967295 [set set_cull_mask; get get_cull_mask]
  The light will affect objects in the selected layers. **Note:** The light cull mask is ignored by VoxelGI, SDFGI, LightmapGI, and volumetric fog. These will always render lights in a way that ignores the cull mask. See also VisualInstance3D.layers.

- light_energy: float = 1.0 [set set_param; get get_param]
  The light's strength multiplier (this is not a physical unit). For OmniLight3D and SpotLight3D, changing this value will only change the light color's intensity, not the light's radius.

- light_indirect_energy: float = 1.0 [set set_param; get get_param]
...
- light_intensity_lumens: float [set set_param; get get_param]
...
- light_intensity_lux: float [set set_param; get get_param]
...
- light_negative: bool = false [set set_negative; get is_negative]
  If true, the light's effect is reversed, darkening areas and casting bright shadows.

- light_projector: Texture2D [set set_projector; get get_projector]
...
- light_size: float = 0.0 [set set_param; get get_param]
...
- light_specular: float = 1.0 [set set_param; get get_param]
  The intensity of the specular blob in objects affected by the light. At 0, the light becomes a pure diffuse light. When not baking emission, this can be used to avoid unrealistic reflections when placing lights above an emissive surface.

- light_temperature: float [set set_temperature; get get_temperature]
...
- light_volumetric_fog_energy: float = 1.0 [set set_param; get get_param]
...
- shadow_bias: float = 0.1 [set set_param; get get_param]
  Used to adjust shadow appearance. Too small a value results in self-shadowing ("shadow acne"), while too large a value causes shadows to separate from casters ("peter-panning"). Adjust as needed.

- shadow_blur: float = 1.0 [set set_param; get get_param]
  Blurs the edges of the shadow. Can be used to hide pixel artifacts in low-resolution shadow maps. A high value can impact performance, make shadows appear grainy and can cause other unwanted artifacts. Try to keep as near default as possible.

- shadow_caster_mask: int = 4294967295 [set set_shadow_caster_mask; get get_shadow_caster_mask]
  The light will only cast shadows using objects in the selected layers.

- shadow_enabled: bool = false [set set_shadow; get has_shadow]
...
- shadow_normal_bias: float = 2.0 [set set_param; get get_param]
...
- shadow_opacity: float = 1.0 [set set_param; get get_param]
...
- shadow_reverse_cull_face: bool = false [set set_shadow_reverse_cull_face; get get_shadow_reverse_cull_face]
...
- shadow_transmittance_bias: float = 0.05 [set set_param; get get_param]
