...
- Source: HeightMapShape3D.xml
- Inherits: Shape3D
- Inheritance Chain: HeightMapShape3D -> Shape3D -> Resource -> RefCounted -> Object
...
```
[methods]
get_max_height() -> float [const]
get_min_height() -> float [const]
...
- map_data: PackedFloat32Array = PackedFloat32Array(0, 0, 0, 0) [set set_map_data; get get_map_data]
  Heightmap data. The array's size must be equal to map_width multiplied by map_depth.

- map_depth: int = 2 [set set_map_depth; get get_map_depth]
  Number of vertices in the depth of the heightmap. Changing this will resize the map_data.

- map_width: int = 2 [set set_map_width; get get_map_width]
