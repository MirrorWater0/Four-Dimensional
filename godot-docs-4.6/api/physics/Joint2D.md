# Joint2D

## Meta

- Name: Joint2D
- Source: Joint2D.xml
- Inherits: Node2D
- Inheritance Chain: Joint2D -> Node2D -> CanvasItem -> Node -> Object

## Brief Description

Abstract base class for all 2D physics joints.

## Description

Abstract base class for all joints in 2D physics. 2D joints bind together two physics bodies (node_a and node_b) and apply a constraint.

## Quick Reference

```
[methods]
get_rid() -> RID [const]

[properties]
bias: float = 0.0
disable_collision: bool = true
node_a: NodePath = NodePath("")
node_b: NodePath = NodePath("")
```

## Methods

- get_rid() -> RID [const]
  Returns the joint's internal RID from the PhysicsServer2D.

## Properties

- bias: float = 0.0 [set set_bias; get get_bias]
  When node_a and node_b move in different directions the bias controls how fast the joint pulls them back to their original position. The lower the bias the more the two bodies can pull on the joint. When set to 0, the default value from ProjectSettings.physics/2d/solver/default_constraint_bias is used.

- disable_collision: bool = true [set set_exclude_nodes_from_collision; get get_exclude_nodes_from_collision]
  If true, the two bodies bound together do not collide with each other.

- node_a: NodePath = NodePath("") [set set_node_a; get get_node_a]
  Path to the first body (A) attached to the joint. The node must inherit PhysicsBody2D.

- node_b: NodePath = NodePath("") [set set_node_b; get get_node_b]
  Path to the second body (B) attached to the joint. The node must inherit PhysicsBody2D.
