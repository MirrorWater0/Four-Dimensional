...
- Name: RigidBody2D
- Source: RigidBody2D.xml
- Inherits: PhysicsBody2D
- Inheritance Chain: RigidBody2D -> PhysicsBody2D -> CollisionObject2D -> Node2D -> CanvasItem -> Node -> Object
...
```
[methods]
_integrate_forces(state: PhysicsDirectBodyState2D) -> void [virtual]
...
apply_central_impulse(impulse: Vector2 = Vector2(0, 0)) -> void
...
get_colliding_bodies() -> Node2D[] [const]
...
- apply_torque(torque: float) -> void
  Applies a rotational force without affecting position. A force is time dependent and meant to be applied every physics update. **Note:** inertia is required for this to work. To have inertia, an active CollisionShape2D must be a child of the node, or you can manually set inertia.
...
- angular_damp_mode: int (RigidBody2D.DampMode) = 0 [set set_angular_damp_mode; get get_angular_damp_mode]
  Defines how angular_damp is applied.

- angular_velocity: float = 0.0 [set set_angular_velocity; get get_angular_velocity]
...
- center_of_mass: Vector2 = Vector2(0, 0) [set set_center_of_mass; get get_center_of_mass]
...
- center_of_mass_mode: int (RigidBody2D.CenterOfMassMode) = 0 [set set_center_of_mass_mode; get get_center_of_mass_mode]
  Defines the way the body's center of mass is set.

- constant_force: Vector2 = Vector2(0, 0) [set set_constant_force; get get_constant_force]
...
- constant_torque: float = 0.0 [set set_constant_torque; get get_constant_torque]
...
- continuous_cd: int (RigidBody2D.CCDMode) = 0 [set set_continuous_collision_detection_mode; get get_continuous_collision_detection_mode]
...
- freeze: bool = false [set set_freeze_enabled; get is_freeze_enabled]
  If true, the body is frozen. Gravity and forces are not applied anymore. See freeze_mode to set the body's behavior when frozen. **Note:** For a body that is always frozen, use StaticBody2D or AnimatableBody2D instead.

- freeze_mode: int (RigidBody2D.FreezeMode) = 0 [set set_freeze_mode; get get_freeze_mode]
  The body's freeze mode. Determines the body's behavior when freeze is true. **Note:** For a body that is always frozen, use StaticBody2D or AnimatableBody2D instead.
...
- inertia: float = 0.0 [set set_inertia; get get_inertia]
...
- linear_damp_mode: int (RigidBody2D.DampMode) = 0 [set set_linear_damp_mode; get get_linear_damp_mode]
  Defines how linear_damp is applied.

- linear_velocity: Vector2 = Vector2(0, 0) [set set_linear_velocity; get get_linear_velocity]
