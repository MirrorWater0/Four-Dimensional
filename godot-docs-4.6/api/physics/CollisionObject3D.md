...
- Source: CollisionObject3D.xml
- Inherits: Node3D
- Inheritance Chain: CollisionObject3D -> Node3D -> Node -> Object
...
```
[methods]
_input_event(camera: Camera3D, event: InputEvent, event_position: Vector3, normal: Vector3, shape_idx: int) -> void [virtual]
_mouse_enter() -> void [virtual]
_mouse_exit() -> void [virtual]
...
is_shape_owner_disabled(owner_id: int) -> bool [const]
...
shape_find_owner(shape_index: int) -> int [const]
shape_owner_add_shape(owner_id: int, shape: Shape3D) -> void
shape_owner_clear_shapes(owner_id: int) -> void
shape_owner_get_owner(owner_id: int) -> Object [const]
shape_owner_get_shape(owner_id: int, shape_id: int) -> Shape3D [const]
shape_owner_get_shape_count(owner_id: int) -> int [const]
shape_owner_get_shape_index(owner_id: int, shape_id: int) -> int [const]
shape_owner_get_transform(owner_id: int) -> Transform3D [const]
shape_owner_remove_shape(owner_id: int, shape_id: int) -> void
shape_owner_set_disabled(owner_id: int, disabled: bool) -> void
...
- collision_priority: float = 1.0 [set set_collision_priority; get get_collision_priority]
  The priority used to solve colliding when occurring penetration. The higher the priority is, the lower the penetration into the object will be. This can for example be used to prevent the player from breaking through the boundaries of a level.

- disable_mode: int (CollisionObject3D.DisableMode) = 0 [set set_disable_mode; get get_disable_mode]
  Defines the behavior in physics when Node.process_mode is set to Node.PROCESS_MODE_DISABLED.

- input_capture_on_drag: bool = false [set set_capture_input_on_drag; get get_capture_input_on_drag]
  If true, the CollisionObject3D will continue to receive input events as the mouse is dragged across its shapes.

- input_ray_pickable: bool = true [set set_ray_pickable; get is_ray_pickable]
...
- mouse_entered()
  Emitted when the mouse pointer enters any of this object's shapes. Requires input_ray_pickable to be true and at least one collision_layer bit to be set. **Note:** Due to the lack of continuous collision detection, this signal may not be emitted in the expected order if the mouse moves fast enough and the CollisionObject3D's area is small. This signal may also not be emitted if another CollisionObject3D is overlapping the CollisionObject3D in question.

- mouse_exited()
  Emitted when the mouse pointer exits all this object's shapes. Requires input_ray_pickable to be true and at least one collision_layer bit to be set. **Note:** Due to the lack of continuous collision detection, this signal may not be emitted in the expected order if the mouse moves fast enough and the CollisionObject3D's area is small. This signal may also not be emitted if another CollisionObject3D is overlapping the CollisionObject3D in question.
