...
- Name: AnimationMixer
- Source: AnimationMixer.xml
- Inherits: Node
- Inheritance Chain: AnimationMixer -> Node -> Object
...
```
[methods]
_post_process_key_value(animation: Animation, track: int, value: Variant, object_id: int, object_sub_idx: int) -> Variant [virtual const]
add_animation_library(name: StringName, library: AnimationLibrary) -> int (Error)
advance(delta: float) -> void
capture(name: StringName, duration: float, trans_type: int (Tween.TransitionType) = 0, ease_type: int (Tween.EaseType) = 0) -> void
clear_caches() -> void
find_animation(animation: Animation) -> StringName [const]
find_animation_library(animation: Animation) -> StringName [const]
get_animation(name: StringName) -> Animation [const]
get_animation_library(name: StringName) -> AnimationLibrary [const]
get_animation_library_list() -> StringName[] [const]
get_animation_list() -> PackedStringArray [const]
get_root_motion_position() -> Vector3 [const]
get_root_motion_position_accumulator() -> Vector3 [const]
get_root_motion_rotation() -> Quaternion [const]
get_root_motion_rotation_accumulator() -> Quaternion [const]
get_root_motion_scale() -> Vector3 [const]
get_root_motion_scale_accumulator() -> Vector3 [const]
has_animation(name: StringName) -> bool [const]
has_animation_library(name: StringName) -> bool [const]
...
- add_animation_library(name: StringName, library: AnimationLibrary) -> int (Error)
  Adds library to the animation player, under the key name. AnimationMixer has a global library by default with an empty string as key. For adding an animation to the global library:
...
- get_animation_library(name: StringName) -> AnimationLibrary [const]
  Returns the first AnimationLibrary with key name or null if not found. To get the AnimationMixer's global animation library, use get_animation_library("").
...
```
  If root_motion_local is true, returns the pre-multiplied translation value with the inverted rotation. In this case, the code can be written as follows:
...
- audio_max_polyphony: int = 32 [set set_audio_max_polyphony; get get_audio_max_polyphony]
...
- callback_mode_discrete: int (AnimationMixer.AnimationCallbackModeDiscrete) = 1 [set set_callback_mode_discrete; get get_callback_mode_discrete]
...
- callback_mode_method: int (AnimationMixer.AnimationCallbackModeMethod) = 0 [set set_callback_mode_method; get get_callback_mode_method]
  The call mode used for "Call Method" tracks.

- callback_mode_process: int (AnimationMixer.AnimationCallbackModeProcess) = 1 [set set_callback_mode_process; get get_callback_mode_process]
  The process notification in which to update animations.

- deterministic: bool = false [set set_deterministic; get is_deterministic]
...
- reset_on_save: bool = true [set set_reset_on_save_enabled; get is_reset_on_save_enabled]
...
- root_motion_local: bool = false [set set_root_motion_local; get is_root_motion_local]
...
- root_motion_track: NodePath = NodePath("") [set set_root_motion_track; get get_root_motion_track]
  The path to the Animation track used for root motion. Paths must be valid scene-tree paths to a node, and must be specified starting from the parent node of the node that will reproduce the animation. The root_motion_track uses the same format as Animation.track_set_path(), but note that a bone must be specified. If the track has type Animation.TYPE_POSITION_3D, Animation.TYPE_ROTATION_3D, or Animation.TYPE_SCALE_3D the transformation will be canceled visually, and the animation will appear to stay in place. See also get_root_motion_position(), get_root_motion_rotation(), get_root_motion_scale(), and RootMotionView.

- root_node: NodePath = NodePath("..") [set set_root_node; get get_root_node]
  The node which node path references will travel from.

## Signals

- animation_finished(anim_name: StringName)
  Notifies when an animation finished playing. **Note:** This signal is not emitted if an animation is looping.
...
- animation_started(anim_name: StringName)
  Notifies when an animation starts playing. **Note:** This signal is not emitted if an animation is looping.
