...
- Name: Animation
- Source: Animation.xml
- Inherits: Resource
- Inheritance Chain: Animation -> Resource -> RefCounted -> Object
...
add_track(type: int (Animation.TrackType), at_position: int = -1) -> int
animation_track_get_key_animation(track_idx: int, key_idx: int) -> StringName [const]
...
audio_track_get_key_end_offset(track_idx: int, key_idx: int) -> float [const]
audio_track_get_key_start_offset(track_idx: int, key_idx: int) -> float [const]
audio_track_get_key_stream(track_idx: int, key_idx: int) -> Resource [const]
audio_track_insert_key(track_idx: int, time: float, stream: Resource, start_offset: float = 0, end_offset: float = 0) -> int
audio_track_is_use_blend(track_idx: int) -> bool [const]
...
audio_track_set_use_blend(track_idx: int, enable: bool) -> void
bezier_track_get_key_in_handle(track_idx: int, key_idx: int) -> Vector2 [const]
bezier_track_get_key_out_handle(track_idx: int, key_idx: int) -> Vector2 [const]
bezier_track_get_key_value(track_idx: int, key_idx: int) -> float [const]
...
bezier_track_interpolate(track_idx: int, time: float) -> float [const]
bezier_track_set_key_in_handle(track_idx: int, key_idx: int, in_handle: Vector2, balanced_value_time_ratio: float = 1.0) -> void
bezier_track_set_key_out_handle(track_idx: int, key_idx: int, out_handle: Vector2, balanced_value_time_ratio: float = 1.0) -> void
...
blend_shape_track_interpolate(track_idx: int, time_sec: float, backward: bool = false) -> float [const]
clear() -> void
compress(page_size: int = 8192, fps: int = 120, split_tolerance: float = 4.0) -> void
copy_track(track_idx: int, to_animation: Animation) -> void
find_track(path: NodePath, type: int (Animation.TrackType)) -> int [const]
get_marker_at_time(time: float) -> StringName [const]
get_marker_color(name: StringName) -> Color [const]
get_marker_names() -> PackedStringArray [const]
get_marker_time(name: StringName) -> float [const]
get_next_marker(time: float) -> StringName [const]
get_prev_marker(time: float) -> StringName [const]
get_track_count() -> int [const]
has_marker(name: StringName) -> bool [const]
method_track_get_name(track_idx: int, key_idx: int) -> StringName [const]
method_track_get_params(track_idx: int, key_idx: int) -> Array [const]
optimize(allowed_velocity_err: float = 0.01, allowed_angular_err: float = 0.01, precision: int = 3) -> void
position_track_insert_key(track_idx: int, time: float, position: Vector3) -> int
position_track_interpolate(track_idx: int, time_sec: float, backward: bool = false) -> Vector3 [const]
...
rotation_track_interpolate(track_idx: int, time_sec: float, backward: bool = false) -> Quaternion [const]
scale_track_insert_key(track_idx: int, time: float, scale: Vector3) -> int
scale_track_interpolate(track_idx: int, time_sec: float, backward: bool = false) -> Vector3 [const]
set_marker_color(name: StringName, color: Color) -> void
track_find_key(track_idx: int, time: float, find_mode: int (Animation.FindMode) = 0, limit: bool = false, backward: bool = false) -> int [const]
track_get_interpolation_loop_wrap(track_idx: int) -> bool [const]
track_get_interpolation_type(track_idx: int) -> int (Animation.InterpolationType) [const]
track_get_key_count(track_idx: int) -> int [const]
track_get_key_time(track_idx: int, key_idx: int) -> float [const]
track_get_key_transition(track_idx: int, key_idx: int) -> float [const]
track_get_key_value(track_idx: int, key_idx: int) -> Variant [const]
track_get_path(track_idx: int) -> NodePath [const]
track_get_type(track_idx: int) -> int (Animation.TrackType) [const]
track_insert_key(track_idx: int, time: float, key: Variant, transition: float = 1) -> int
track_is_compressed(track_idx: int) -> bool [const]
track_is_enabled(track_idx: int) -> bool [const]
track_is_imported(track_idx: int) -> bool [const]
...
track_set_enabled(track_idx: int, enabled: bool) -> void
track_set_imported(track_idx: int, imported: bool) -> void
track_set_interpolation_loop_wrap(track_idx: int, interpolation: bool) -> void
track_set_interpolation_type(track_idx: int, interpolation: int (Animation.InterpolationType)) -> void
...
track_set_key_value(track_idx: int, key: int, value: Variant) -> void
track_set_path(track_idx: int, path: NodePath) -> void
track_swap(track_idx: int, with_idx: int) -> void
value_track_get_update_mode(track_idx: int) -> int (Animation.UpdateMode) [const]
value_track_interpolate(track_idx: int, time_sec: float, backward: bool = false) -> Variant [const]
value_track_set_update_mode(track_idx: int, mode: int (Animation.UpdateMode)) -> void
...
- capture_included: bool = false [get is_capture_included]
...
- loop_mode: int (Animation.LoopMode) = 0 [set set_loop_mode; get get_loop_mode]
...
- step: float = 0.033333335 [set set_step; get get_step]
...
- INTERPOLATION_CUBIC_ANGLE = 4
  Cubic interpolation with shortest path rotation. **Note:** The result value is always normalized and may not match the key value.
