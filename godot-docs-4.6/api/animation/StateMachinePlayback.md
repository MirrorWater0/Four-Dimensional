...
- Source: AnimationNodeStateMachinePlayback.xml
- Inherits: Resource
- Inheritance Chain: AnimationNodeStateMachinePlayback -> Resource -> RefCounted -> Object
...
```
[methods]
get_current_length() -> float [const]
get_current_node() -> StringName [const]
get_current_play_position() -> float [const]
get_fading_from_length() -> float [const]
get_fading_from_node() -> StringName [const]
get_fading_from_play_position() -> float [const]
get_fading_length() -> float [const]
get_fading_position() -> float [const]
get_travel_path() -> StringName[] [const]
...
- start(node: StringName, reset: bool = true) -> void
  Starts playing the given animation. If reset is true, the animation is played from the beginning.

- stop() -> void
  Stops the currently playing animation.

- travel(to_node: StringName, reset_on_teleport: bool = true) -> void
  Transitions from the current state to another one, following the shortest path. If the path does not connect from the current state, the animation will play after the state teleports. If reset_on_teleport is true, the animation is played from the beginning when the travel cause a teleportation.
...
- state_finished(state: StringName)
  Emitted when the state finishes playback. If state is a state machine set to grouped mode, its signals are passed through with its name prefixed. If there is a crossfade, this will be fired when the influence of the get_fading_from_node() animation is no longer present.
