...
- Name: MultiplayerAPI
- Source: MultiplayerAPI.xml
- Inherits: RefCounted
- Inheritance Chain: MultiplayerAPI -> RefCounted -> Object
...
```
[methods]
create_default_interface() -> MultiplayerAPI [static]
get_default_interface() -> StringName [static]
get_peers() -> PackedInt32Array
get_remote_sender_id() -> int
get_unique_id() -> int
has_multiplayer_peer() -> bool
is_server() -> bool
object_configuration_add(object: Object, configuration: Variant) -> int (Error)
object_configuration_remove(object: Object, configuration: Variant) -> int (Error)
poll() -> int (Error)
rpc(peer: int, object: Object, method: StringName, arguments: Array = []) -> int (Error)
set_default_interface(interface_name: StringName) -> void [static]
...
- get_remote_sender_id() -> int
  Returns the sender's peer ID for the RPC currently being executed. **Note:** This method returns 0 when called outside of an RPC. As such, the original peer ID may be lost when code execution is delayed (such as with GDScript's await keyword).
...
- multiplayer_peer: MultiplayerPeer [set set_multiplayer_peer; get get_multiplayer_peer]
