...
- Name: PacketPeer
- Source: PacketPeer.xml
- Inherits: RefCounted
- Inheritance Chain: PacketPeer -> RefCounted -> Object
...
```
[methods]
get_available_packet_count() -> int [const]
get_packet() -> PackedByteArray
get_packet_error() -> int (Error) [const]
get_var(allow_objects: bool = false) -> Variant
put_packet(buffer: PackedByteArray) -> int (Error)
put_var(var: Variant, full_objects: bool = false) -> int (Error)
...
- put_var(var: Variant, full_objects: bool = false) -> int (Error)
  Sends a Variant as a packet. If full_objects is true, encoding objects is allowed (and can potentially include code). Internally, this uses the same encoding mechanism as the @GlobalScope.var_to_bytes() method.

## Properties

- encode_buffer_max_size: int = 8388608 [set set_encode_buffer_max_size; get get_encode_buffer_max_size]
  Maximum buffer size allowed when encoding Variants. Raise this value to support heavier memory allocations. The put_var() method allocates memory on the stack, and the buffer used will grow automatically to the closest power of two to match the size of the Variant. If the Variant is bigger than encode_buffer_max_size, the method will error out with ERR_OUT_OF_MEMORY.
