...
- Source: Resource.xml
- Inherits: RefCounted
- Inheritance Chain: Resource -> RefCounted -> Object
...
```
[methods]
_get_rid() -> RID [virtual const]
_reset_state() -> void [virtual]
_set_path_cache(path: String) -> void [virtual const]
_setup_local_to_scene() -> void [virtual]
duplicate(deep: bool = false) -> Resource [const]
duplicate_deep(deep_subresources_mode: int (Resource.DeepDuplicateMode) = 1) -> Resource [const]
emit_changed() -> void
generate_scene_unique_id() -> String [static]
get_id_for_path(path: String) -> String [const]
get_local_scene() -> Node [const]
get_rid() -> RID [const]
is_built_in() -> bool [const]
reset_state() -> void
...
- _setup_local_to_scene() -> void [virtual]
  Override this method to customize the newly duplicated resource created from PackedScene.instantiate(), if the original's resource_local_to_scene is set to true. **Example:** Set a random damage value to every local resource from an instantiated scene:
...
- resource_local_to_scene: bool = false [set set_local_to_scene; get is_local_to_scene]
...
- resource_name: String = "" [set set_name; get get_name]
...
- resource_path: String = "" [set set_path; get get_path]
...
- resource_scene_unique_id: String [set set_scene_unique_id; get get_scene_unique_id]
  A unique identifier relative to the this resource's scene. If left empty, the ID is automatically generated when this resource is saved inside a PackedScene. If the resource is not inside a scene, this property is empty by default. **Note:** When the PackedScene is saved, if multiple resources in the same scene use the same ID, only the earliest resource in the scene hierarchy keeps the original ID. The other resources are assigned new IDs from generate_scene_unique_id(). **Note:** Setting this property does not emit the changed signal. **Warning:** When setting, the ID must only consist of letters, numbers, and underscores. Otherwise, it will fail and default to a randomly generated ID.

## Signals

- changed()
  Emitted when the resource changes, usually when one of its properties is modified. See also emit_changed(). **Note:** This signal is not emitted automatically for properties of custom resources. If necessary, a setter needs to be created to emit the signal.
