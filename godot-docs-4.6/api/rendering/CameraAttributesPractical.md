...
- Source: CameraAttributesPractical.xml
- Inherits: CameraAttributes
- Inheritance Chain: CameraAttributesPractical -> CameraAttributes -> Resource -> RefCounted -> Object
...
- auto_exposure_max_sensitivity: float = 800.0 [set set_auto_exposure_max_sensitivity; get get_auto_exposure_max_sensitivity]
...
- auto_exposure_min_sensitivity: float = 0.0 [set set_auto_exposure_min_sensitivity; get get_auto_exposure_min_sensitivity]
...
- dof_blur_amount: float = 0.1 [set set_dof_blur_amount; get get_dof_blur_amount]
...
- dof_blur_far_distance: float = 10.0 [set set_dof_blur_far_distance; get get_dof_blur_far_distance]
  Objects further from the Camera3D by this amount will be blurred by the depth of field effect. Measured in meters.

- dof_blur_far_enabled: bool = false [set set_dof_blur_far_enabled; get is_dof_blur_far_enabled]
  Enables depth of field blur for objects further than dof_blur_far_distance. Strength of blur is controlled by dof_blur_amount and modulated by dof_blur_far_transition. **Note:** Depth of field blur is only supported in the Forward+ and Mobile rendering methods, not Compatibility. **Note:** Depth of field blur is not supported on viewports that have a transparent background (where Viewport.transparent_bg is true).

- dof_blur_far_transition: float = 5.0 [set set_dof_blur_far_transition; get get_dof_blur_far_transition]
...
- dof_blur_near_distance: float = 2.0 [set set_dof_blur_near_distance; get get_dof_blur_near_distance]
  Objects closer from the Camera3D by this amount will be blurred by the depth of field effect. Measured in meters.

- dof_blur_near_enabled: bool = false [set set_dof_blur_near_enabled; get is_dof_blur_near_enabled]
...
- dof_blur_near_transition: float = 1.0 [set set_dof_blur_near_transition; get get_dof_blur_near_transition]
