...
- Source: ImageTextureLayered.xml
- Inherits: TextureLayered
- Inheritance Chain: ImageTextureLayered -> TextureLayered -> Texture -> Resource -> RefCounted -> Object
...
```
[methods]
create_from_images(images: Image[]) -> int (Error)
...
- create_from_images(images: Image[]) -> int (Error)
  Creates an ImageTextureLayered from an array of Images. See Image.create() for the expected data format. The first image decides the width, height, image format and mipmapping setting. The other images *must* have the same width, height, image format and mipmapping setting. Each Image represents one layer.
