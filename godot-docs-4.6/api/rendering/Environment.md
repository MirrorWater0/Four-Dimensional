...
- Inherits: Resource
- Inheritance Chain: Environment -> Resource -> RefCounted -> Object
...
```
[methods]
get_glow_level(idx: int) -> float [const]
...
- adjustment_brightness: float = 1.0 [set set_adjustment_brightness; get get_adjustment_brightness]
...
- adjustment_color_correction: Texture [set set_adjustment_color_correction; get get_adjustment_color_correction]
...
- adjustment_contrast: float = 1.0 [set set_adjustment_contrast; get get_adjustment_contrast]
...
- adjustment_enabled: bool = false [set set_adjustment_enabled; get is_adjustment_enabled]
  If true, enables the adjustment_* properties provided by this resource. If false, modifications to the adjustment_* properties will have no effect on the rendered scene.

- adjustment_saturation: float = 1.0 [set set_adjustment_saturation; get get_adjustment_saturation]
...
- ambient_light_color: Color = Color(0, 0, 0, 1) [set set_ambient_light_color; get get_ambient_light_color]
  The ambient light's Color. Only effective if ambient_light_sky_contribution is lower than 1.0 (exclusive).

- ambient_light_energy: float = 1.0 [set set_ambient_light_energy; get get_ambient_light_energy]
...
- ambient_light_sky_contribution: float = 1.0 [set set_ambient_light_sky_contribution; get get_ambient_light_sky_contribution]
...
- ambient_light_source: int (Environment.AmbientSource) = 0 [set set_ambient_source; get get_ambient_source]
  The ambient light source to use for rendering materials and global illumination.

- background_camera_feed_id: int = 1 [set set_camera_feed_id; get get_camera_feed_id]
  The ID of the camera feed to show in the background.

- background_canvas_max_layer: int = 0 [set set_canvas_max_layer; get get_canvas_max_layer]
  The maximum layer ID to display. Only effective when using the BG_CANVAS background mode.

- background_color: Color = Color(0, 0, 0, 1) [set set_bg_color; get get_bg_color]
  The Color displayed for clear areas of the scene. Only effective when using the BG_COLOR background mode.

- background_energy_multiplier: float = 1.0 [set set_bg_energy_multiplier; get get_bg_energy_multiplier]
  Multiplier for background energy. Increase to make background brighter, decrease to make background dimmer.

- background_intensity: float = 30000.0 [set set_bg_intensity; get get_bg_intensity]
...
- background_mode: int (Environment.BGMode) = 0 [set set_background; get get_background]
  The background mode.

- fog_aerial_perspective: float = 0.0 [set set_fog_aerial_perspective; get get_fog_aerial_perspective]
  If set above 0.0 (exclusive), blends between the fog's color and the color of the background Sky, as read from the radiance cubemap. This has a small performance cost when set above 0.0. Must have background_mode set to BG_SKY. This is useful to simulate [aerial perspective](https://en.wikipedia.org/wiki/Aerial_perspective) in large scenes with low density fog. However, it is not very useful for high-density fog, as the sky will shine through. When set to 1.0, the fog color comes completely from the Sky. If set to 0.0, aerial perspective is disabled. Notice that this does not sample the Sky directly, but rather the radiance cubemap. The cubemap is sampled at a mipmap level depending on the depth of the rendered pixel; the farther away, the higher the resolution of the sampled mipmap. This results in the actual color being a blurred version of the sky, with more blur closer to the camera. The highest mipmap resolution is used at a depth of Camera3D.far.

- fog_density: float = 0.01 [set set_fog_density; get get_fog_density]
  The fog density to be used. This is demonstrated in different ways depending on the fog_mode mode chosen: **Exponential Fog Mode:** Higher values result in denser fog. The fog rendering is exponential like in real life. **Depth Fog mode:** The maximum intensity of the deep fog, effect will appear in the distance (relative to the camera). At 1.0 the fog will fully obscure the scene, at 0.0 the fog will not be visible.

- fog_depth_begin: float = 10.0 [set set_fog_depth_begin; get get_fog_depth_begin]
  The fog's depth starting distance from the camera. Only available when fog_mode is set to FOG_MODE_DEPTH.

- fog_depth_curve: float = 1.0 [set set_fog_depth_curve; get get_fog_depth_curve]
...
- fog_depth_end: float = 100.0 [set set_fog_depth_end; get get_fog_depth_end]
...
- fog_enabled: bool = false [set set_fog_enabled; get is_fog_enabled]
  If true, fog effects are enabled.

- fog_height: float = 0.0 [set set_fog_height; get get_fog_height]
  The height at which the height fog effect begins.

- fog_height_density: float = 0.0 [set set_fog_height_density; get get_fog_height_density]
  The density used to increase fog as height decreases. To make fog increase as height increases, use a negative value.

- fog_light_color: Color = Color(0.518, 0.553, 0.608, 1) [set set_fog_light_color; get get_fog_light_color]
  The fog's color.

- fog_light_energy: float = 1.0 [set set_fog_light_energy; get get_fog_light_energy]
  The fog's brightness. Higher values result in brighter fog.

- fog_mode: int (Environment.FogMode) = 0 [set set_fog_mode; get get_fog_mode]
  The fog mode.

- fog_sky_affect: float = 1.0 [set set_fog_sky_affect; get get_fog_sky_affect]
...
- fog_sun_scatter: float = 0.0 [set set_fog_sun_scatter; get get_fog_sun_scatter]
...
- glow_blend_mode: int (Environment.GlowBlendMode) = 1 [set set_glow_blend_mode; get get_glow_blend_mode]
  The glow blending mode. **Note:** The Compatibility renderer always uses GLOW_BLEND_MODE_SCREEN and glow_blend_mode will have no effect.

- glow_bloom: float = 0.0 [set set_glow_bloom; get get_glow_bloom]
...
- glow_enabled: bool = false [set set_glow_enabled; get is_glow_enabled]
...
- glow_hdr_luminance_cap: float = 12.0 [set set_glow_hdr_luminance_cap; get get_glow_hdr_luminance_cap]
  The higher threshold of the HDR glow. Areas brighter than this threshold will be clamped for the purposes of the glow effect.

- glow_hdr_scale: float = 2.0 [set set_glow_hdr_bleed_scale; get get_glow_hdr_bleed_scale]
...
- glow_hdr_threshold: float = 1.0 [set set_glow_hdr_bleed_threshold; get get_glow_hdr_bleed_threshold]
...
- glow_intensity: float = 0.3 [set set_glow_intensity; get get_glow_intensity]
...
- glow_levels/1: float = 0.0 [set set_glow_level; get get_glow_level]
  The intensity of the 1st level of glow. This is the most "local" level (least blurry). **Note:** glow_levels/1 has no effect when using the Compatibility rendering method, due to this rendering method using a simpler glow implementation optimized for low-end devices.

- glow_levels/2: float = 0.8 [set set_glow_level; get get_glow_level]
  The intensity of the 2nd level of glow. **Note:** glow_levels/2 has no effect when using the Compatibility rendering method, due to this rendering method using a simpler glow implementation optimized for low-end devices.

- glow_levels/3: float = 0.4 [set set_glow_level; get get_glow_level]
  The intensity of the 3rd level of glow. **Note:** glow_levels/3 has no effect when using the Compatibility rendering method, due to this rendering method using a simpler glow implementation optimized for low-end devices.

- glow_levels/4: float = 0.1 [set set_glow_level; get get_glow_level]
  The intensity of the 4th level of glow. **Note:** glow_levels/4 has no effect when using the Compatibility rendering method, due to this rendering method using a simpler glow implementation optimized for low-end devices.

- glow_levels/5: float = 0.0 [set set_glow_level; get get_glow_level]
  The intensity of the 5th level of glow. **Note:** glow_levels/5 has no effect when using the Compatibility rendering method, due to this rendering method using a simpler glow implementation optimized for low-end devices.

- glow_levels/6: float = 0.0 [set set_glow_level; get get_glow_level]
  The intensity of the 6th level of glow. **Note:** glow_levels/6 has no effect when using the Compatibility rendering method, due to this rendering method using a simpler glow implementation optimized for low-end devices.

- glow_levels/7: float = 0.0 [set set_glow_level; get get_glow_level]
  The intensity of the 7th level of glow. This is the most "global" level (blurriest). **Note:** glow_levels/7 has no effect when using the Compatibility rendering method, due to this rendering method using a simpler glow implementation optimized for low-end devices.

- glow_map: Texture [set set_glow_map; get get_glow_map]
  The texture that should be used as a glow map to *multiply* the resulting glow color according to glow_map_strength. This can be used to create a "lens dirt" effect. The texture's RGB color channels are used for modulation, but the alpha channel is ignored. **Note:** The texture will be stretched to fit the screen. Therefore, it's recommended to use a texture with an aspect ratio that matches your project's base aspect ratio (typically 16:9). **Note:** glow_map has no effect when using the Compatibility rendering method, due to this rendering method using a simpler glow implementation optimized for low-end devices.

- glow_map_strength: float = 0.8 [set set_glow_map_strength; get get_glow_map_strength]
  How strong of an influence the glow_map should have on the overall glow effect. A strength of 0.0 means the glow map has no influence, while a strength of 1.0 means the glow map has full influence. **Note:** If the glow map has black areas, a value of 1.0 can also turn off the glow effect entirely in specific areas of the screen. **Note:** glow_map_strength has no effect when using the Compatibility rendering method, due to this rendering method using a simpler glow implementation optimized for low-end devices.

- glow_mix: float = 0.05 [set set_glow_mix; get get_glow_mix]
  When using the GLOW_BLEND_MODE_MIX glow_blend_mode, this controls how much the source image is blended with the glow layer. A value of 0.0 makes the glow rendering invisible, while a value of 1.0 is equivalent to GLOW_BLEND_MODE_REPLACE. **Note:** glow_mix has no effect when using the Compatibility rendering method, due to this rendering method using a simpler glow implementation optimized for low-end devices.

- glow_normalized: bool = false [set set_glow_normalized; get is_glow_normalized]
  If true, glow levels will be normalized so that summed together their intensities equal 1.0. **Note:** glow_normalized has no effect when using the Compatibility rendering method, due to this rendering method using a simpler glow implementation optimized for low-end devices.

- glow_strength: float = 1.0 [set set_glow_strength; get get_glow_strength]
  The strength that is used when blurring across the screen to generate the glow effect. This affects the distance and intensity of the blur. When using the Mobile rendering method, this should be increased to compensate for the lower dynamic range. **Note:** glow_strength has no effect when using the Compatibility rendering method, due to this rendering method using a simpler glow implementation optimized for low-end devices.

- reflected_light_source: int (Environment.ReflectionSource) = 0 [set set_reflection_source; get get_reflection_source]
  The reflected (specular) light source.

- sdfgi_bounce_feedback: float = 0.5 [set set_sdfgi_bounce_feedback; get get_sdfgi_bounce_feedback]
...
- sdfgi_cascade0_distance: float = 12.8 [set set_sdfgi_cascade0_distance; get get_sdfgi_cascade0_distance]
...
- sdfgi_cascades: int = 4 [set set_sdfgi_cascades; get get_sdfgi_cascades]
...
- sdfgi_enabled: bool = false [set set_sdfgi_enabled; get is_sdfgi_enabled]
...
- sdfgi_energy: float = 1.0 [set set_sdfgi_energy; get get_sdfgi_energy]
...
- sdfgi_max_distance: float = 204.8 [set set_sdfgi_max_distance; get get_sdfgi_max_distance]
...
- sdfgi_min_cell_size: float = 0.2 [set set_sdfgi_min_cell_size; get get_sdfgi_min_cell_size]
...
- sdfgi_normal_bias: float = 1.1 [set set_sdfgi_normal_bias; get get_sdfgi_normal_bias]
...
- sdfgi_probe_bias: float = 1.1 [set set_sdfgi_probe_bias; get get_sdfgi_probe_bias]
...
- sdfgi_read_sky_light: bool = true [set set_sdfgi_read_sky_light; get is_sdfgi_reading_sky_light]
  If true, SDFGI takes the environment lighting into account. This should be set to false for interior scenes.

- sdfgi_use_occlusion: bool = false [set set_sdfgi_use_occlusion; get is_sdfgi_using_occlusion]
...
- sdfgi_y_scale: int (Environment.SDFGIYScale) = 1 [set set_sdfgi_y_scale; get get_sdfgi_y_scale]
...
- sky: Sky [set set_sky; get get_sky]
  The Sky resource used for this Environment.

- sky_custom_fov: float = 0.0 [set set_sky_custom_fov; get get_sky_custom_fov]
...
- sky_rotation: Vector3 = Vector3(0, 0, 0) [set set_sky_rotation; get get_sky_rotation]
  The rotation to use for sky rendering.

- ssao_ao_channel_affect: float = 0.0 [set set_ssao_ao_channel_affect; get get_ssao_ao_channel_affect]
  The screen-space ambient occlusion intensity on materials that have an AO texture defined. Values higher than 0 will make the SSAO effect visible in areas darkened by AO textures.

- ssao_detail: float = 0.5 [set set_ssao_detail; get get_ssao_detail]
  Sets the strength of the additional level of detail for the screen-space ambient occlusion effect. A high value makes the detail pass more prominent, but it may contribute to aliasing in your final image.

- ssao_enabled: bool = false [set set_ssao_enabled; get is_ssao_enabled]
...
- ssao_horizon: float = 0.06 [set set_ssao_horizon; get get_ssao_horizon]
...
- ssao_intensity: float = 2.0 [set set_ssao_intensity; get get_ssao_intensity]
  The primary screen-space ambient occlusion intensity. Acts as a multiplier for the screen-space ambient occlusion effect. A higher value results in darker occlusion.

- ssao_light_affect: float = 0.0 [set set_ssao_direct_light_affect; get get_ssao_direct_light_affect]
  The screen-space ambient occlusion intensity in direct light. In real life, ambient occlusion only applies to indirect light, which means its effects can't be seen in direct light. Values higher than 0 will make the SSAO effect visible in direct light.

- ssao_power: float = 1.5 [set set_ssao_power; get get_ssao_power]
...
- ssao_radius: float = 1.0 [set set_ssao_radius; get get_ssao_radius]
  The distance at which objects can occlude each other when calculating screen-space ambient occlusion. Higher values will result in occlusion over a greater distance at the cost of performance and quality.

- ssao_sharpness: float = 0.98 [set set_ssao_sharpness; get get_ssao_sharpness]
...
- ssil_enabled: bool = false [set set_ssil_enabled; get is_ssil_enabled]
...
- ssil_intensity: float = 1.0 [set set_ssil_intensity; get get_ssil_intensity]
  The brightness multiplier for the screen-space indirect lighting effect. A higher value will result in brighter light.

- ssil_normal_rejection: float = 1.0 [set set_ssil_normal_rejection; get get_ssil_normal_rejection]
  Amount of normal rejection used when calculating screen-space indirect lighting. Normal rejection uses the normal of a given sample point to reject samples that are facing away from the current pixel. Normal rejection is necessary to avoid light leaking when only one side of an object is illuminated. However, normal rejection can be disabled if light leaking is desirable, such as when the scene mostly contains emissive objects that emit light from faces that cannot be seen from the camera.

- ssil_radius: float = 5.0 [set set_ssil_radius; get get_ssil_radius]
...
- ssil_sharpness: float = 0.98 [set set_ssil_sharpness; get get_ssil_sharpness]
  The amount that the screen-space indirect lighting effect is allowed to blur over the edges of objects. Setting too high will result in aliasing around the edges of objects. Setting too low will make object edges appear blurry.

- ssr_depth_tolerance: float = 0.5 [set set_ssr_depth_tolerance; get get_ssr_depth_tolerance]
  The depth tolerance for screen-space reflections.

- ssr_enabled: bool = false [set set_ssr_enabled; get is_ssr_enabled]
  If true, screen-space reflections are enabled. Screen-space reflections are more accurate than reflections from VoxelGIs or ReflectionProbes, but are slower and can't reflect surfaces occluded by others. **Note:** SSR is only supported in the Forward+ rendering method, not Mobile or Compatibility. **Note:** SSR is not supported on viewports that have a transparent background (where Viewport.transparent_bg is true).

- ssr_fade_in: float = 0.15 [set set_ssr_fade_in; get get_ssr_fade_in]
...
- ssr_fade_out: float = 2.0 [set set_ssr_fade_out; get get_ssr_fade_out]
...
- ssr_max_steps: int = 64 [set set_ssr_max_steps; get get_ssr_max_steps]
  The maximum number of steps for screen-space reflections. Higher values are slower.

- tonemap_agx_contrast: float = 1.25 [set set_tonemap_agx_contrast; get get_tonemap_agx_contrast]
...
- tonemap_mode: int (Environment.ToneMapper) = 0 [set set_tonemapper; get get_tonemapper]
...
- tonemap_white: float = 1.0 [set set_tonemap_white; get get_tonemap_white]
  The white reference value for tonemapping, which indicates where bright white is located in the scale of values provided to the tonemapper. For photorealistic lighting, it is recommended to set tonemap_white to at least 6.0. Higher values result in less blown out highlights, but may make the scene appear lower contrast. tonemap_agx_white will be used instead when using the TONE_MAPPER_AGX tonemapper. See also tonemap_exposure. **Note:** tonemap_white must be set to 2.0 or lower on the Mobile renderer to produce bright images.

- volumetric_fog_albedo: Color = Color(1, 1, 1, 1) [set set_volumetric_fog_albedo; get get_volumetric_fog_albedo]
...
- volumetric_fog_ambient_inject: float = 0.0 [set set_volumetric_fog_ambient_inject; get get_volumetric_fog_ambient_inject]
...
- volumetric_fog_anisotropy: float = 0.2 [set set_volumetric_fog_anisotropy; get get_volumetric_fog_anisotropy]
...
- volumetric_fog_density: float = 0.05 [set set_volumetric_fog_density; get get_volumetric_fog_density]
...
- volumetric_fog_detail_spread: float = 2.0 [set set_volumetric_fog_detail_spread; get get_volumetric_fog_detail_spread]
  The distribution of size down the length of the froxel buffer. A higher value compresses the froxels closer to the camera and places more detail closer to the camera.

- volumetric_fog_emission: Color = Color(0, 0, 0, 1) [set set_volumetric_fog_emission; get get_volumetric_fog_emission]
...
- volumetric_fog_emission_energy: float = 1.0 [set set_volumetric_fog_emission_energy; get get_volumetric_fog_emission_energy]
  The brightness of the emitted light from the volumetric fog.

- volumetric_fog_enabled: bool = false [set set_volumetric_fog_enabled; get is_volumetric_fog_enabled]
...
- volumetric_fog_gi_inject: float = 1.0 [set set_volumetric_fog_gi_inject; get get_volumetric_fog_gi_inject]
...
- volumetric_fog_length: float = 64.0 [set set_volumetric_fog_length; get get_volumetric_fog_length]
...
- volumetric_fog_sky_affect: float = 1.0 [set set_volumetric_fog_sky_affect; get get_volumetric_fog_sky_affect]
...
- volumetric_fog_temporal_reprojection_amount: float = 0.9 [set set_volumetric_fog_temporal_reprojection_amount; get get_volumetric_fog_temporal_reprojection_amount]
  The amount by which to blend the last frame with the current frame. A higher number results in smoother volumetric fog, but makes "ghosting" much worse. A lower value reduces ghosting but can result in the per-frame temporal jitter becoming visible.

- volumetric_fog_temporal_reprojection_enabled: bool = true [set set_volumetric_fog_temporal_reprojection_enabled; get is_volumetric_fog_temporal_reprojection_enabled]
...
- TONE_MAPPER_ACES = 3
  Uses a high-contrast film-like tonemapping curve and desaturates bright values for a more realistic appearance. Slightly slower than TONE_MAPPER_FILMIC. **Note:** This tonemapping operator is called "ACES Fitted" in Godot 3.x.
