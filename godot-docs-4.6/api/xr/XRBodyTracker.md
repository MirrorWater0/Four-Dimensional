...
- Name: XRBodyTracker
- Source: XRBodyTracker.xml
- Inherits: XRPositionalTracker
- Inheritance Chain: XRBodyTracker -> XRPositionalTracker -> XRTracker -> RefCounted -> Object
...
```
[methods]
get_joint_flags(joint: int (XRBodyTracker.Joint)) -> int (XRBodyTracker.JointFlags) [const]
get_joint_transform(joint: int (XRBodyTracker.Joint)) -> Transform3D [const]
set_joint_flags(joint: int (XRBodyTracker.Joint), flags: int (XRBodyTracker.JointFlags)) -> void
set_joint_transform(joint: int (XRBodyTracker.Joint), transform: Transform3D) -> void
...
- body_flags: int (XRBodyTracker.BodyFlags) = 0 [set set_body_flags; get get_body_flags]
...
- type: int (XRServer.TrackerType) = 32 [set set_tracker_type; get get_tracker_type; override XRTracker]

## Constants

### Enum BodyFlags

- BODY_FLAG_UPPER_BODY_SUPPORTED = 1 [bitfield]
  Upper body tracking supported.

- BODY_FLAG_LOWER_BODY_SUPPORTED = 2 [bitfield]
  Lower body tracking supported.

- BODY_FLAG_HANDS_SUPPORTED = 4 [bitfield]
...
- JOINT_FLAG_ORIENTATION_VALID = 1 [bitfield]
  The joint's orientation data is valid.

- JOINT_FLAG_ORIENTATION_TRACKED = 2 [bitfield]
  The joint's orientation is actively tracked. May not be set if tracking has been temporarily lost.

- JOINT_FLAG_POSITION_VALID = 4 [bitfield]
  The joint's position data is valid.

- JOINT_FLAG_POSITION_TRACKED = 8 [bitfield]
