...
- Source: Engine.xml
- Inherits: Object
- Inheritance Chain: Engine -> Object
...
```
[methods]
capture_script_backtraces(include_variables: bool = false) -> ScriptBacktrace[] [const]
get_architecture_name() -> String [const]
get_author_info() -> Dictionary [const]
get_copyright_info() -> Dictionary[] [const]
get_donor_info() -> Dictionary [const]
get_frames_drawn() -> int
get_frames_per_second() -> float [const]
get_license_info() -> Dictionary [const]
get_license_text() -> String [const]
get_main_loop() -> MainLoop [const]
get_physics_frames() -> int [const]
get_physics_interpolation_fraction() -> float [const]
get_process_frames() -> int [const]
get_script_language(index: int) -> ScriptLanguage [const]
get_script_language_count() -> int
get_singleton(name: StringName) -> Object [const]
get_singleton_list() -> PackedStringArray [const]
get_version_info() -> Dictionary [const]
get_write_movie_path() -> String [const]
has_singleton(name: StringName) -> bool [const]
is_editor_hint() -> bool [const]
is_embedded_in_editor() -> bool [const]
is_in_physics_frame() -> bool [const]
register_script_language(language: ScriptLanguage) -> int (Error)
register_singleton(name: StringName, instance: Object) -> void
unregister_script_language(language: ScriptLanguage) -> int (Error)
...
- get_frames_drawn() -> int
  Returns the total number of frames drawn since the engine started. **Note:** On headless platforms, or if rendering is disabled with --disable-render-loop via command line, this method always returns 0. See also get_process_frames().
...
- get_main_loop() -> MainLoop [const]
  Returns the instance of the MainLoop. This is usually the main SceneTree and is the same as Node.get_tree(). **Note:** The type instantiated as the main loop can changed with ProjectSettings.application/run/main_loop_type.

- get_physics_frames() -> int [const]
  Returns the total number of frames passed since the engine started. This number is increased every **physics frame**. See also get_process_frames(). This method can be used to run expensive logic less often without relying on a Timer:
...
- get_process_frames() -> int [const]
  Returns the total number of frames passed since the engine started. This number is increased every **process frame**, regardless of whether the render loop is enabled. See also get_frames_drawn() and get_physics_frames(). This method can be used to run expensive logic less often without relying on a Timer:
...
```
  See [Running code in the editor]($DOCS_URL/tutorials/plugins/running_code_in_the_editor.html) in the documentation for more information. **Note:** To detect whether the script is running on an editor *build* (such as when pressing F5), use OS.has_feature() with the "editor" argument instead. OS.has_feature("editor") evaluate to true both when the script is running in the editor and when running the project from the editor, but returns false when run from an exported project.
...
- register_script_language(language: ScriptLanguage) -> int (Error)
  Registers a ScriptLanguage instance to be available with ScriptServer. Returns: - OK on success; - ERR_UNAVAILABLE if ScriptServer has reached the limit and cannot register any new language; - ERR_ALREADY_EXISTS if ScriptServer already contains a language with similar extension/name/type.

- register_singleton(name: StringName, instance: Object) -> void
  Registers the given Object instance as a singleton, available globally under name. Useful for plugins.

- unregister_script_language(language: ScriptLanguage) -> int (Error)
  Unregisters the ScriptLanguage instance from ScriptServer. Returns: - OK on success; - ERR_DOES_NOT_EXIST if the language is not registered in ScriptServer.
