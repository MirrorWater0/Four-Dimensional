...
- Name: GraphEdit
- Source: GraphEdit.xml
- Inherits: Control
- Inheritance Chain: GraphEdit -> Control -> CanvasItem -> Node -> Object
...
```
[methods]
_get_connection_line(from_position: Vector2, to_position: Vector2) -> PackedVector2Array [virtual const]
_is_in_input_hotzone(in_node: Object, in_port: int, mouse_position: Vector2) -> bool [virtual]
_is_in_output_hotzone(in_node: Object, in_port: int, mouse_position: Vector2) -> bool [virtual]
_is_node_hover_valid(from_node: StringName, from_port: int, to_node: StringName, to_port: int) -> bool [virtual]
...
connect_node(from_node: StringName, from_port: int, to_node: StringName, to_port: int, keep_alive: bool = false) -> int (Error)
...
get_closest_connection_at_point(point: Vector2, max_distance: float = 4.0) -> Dictionary [const]
get_connection_count(from_node: StringName, from_port: int) -> int
get_connection_line(from_node: Vector2, to_node: Vector2) -> PackedVector2Array [const]
get_connection_list_from_node(node: StringName) -> Dictionary[] [const]
get_connections_intersecting_with_rect(rect: Rect2) -> Dictionary[] [const]
...
is_valid_connection_type(from_type: int, to_type: int) -> bool [const]
...
- add_valid_connection_type(from_type: int, to_type: int) -> void
  Allows the connection between two different port types. The port type is defined individually for the left and the right port of each slot with the GraphNode.set_slot() method. See also is_valid_connection_type() and remove_valid_connection_type().
...
- is_valid_connection_type(from_type: int, to_type: int) -> bool [const]
  Returns whether it's possible to make a connection between two different port types. The port type is defined individually for the left and the right port of each slot with the GraphNode.set_slot() method. See also add_valid_connection_type() and remove_valid_connection_type().

- remove_valid_connection_type(from_type: int, to_type: int) -> void
  Disallows the connection between two different port types previously allowed by add_valid_connection_type(). The port type is defined individually for the left and the right port of each slot with the GraphNode.set_slot() method. See also is_valid_connection_type().
...
- connection_lines_antialiased: bool = true [set set_connection_lines_antialiased; get is_connection_lines_antialiased]
  If true, the lines between nodes will use antialiasing.

- connection_lines_curvature: float = 0.5 [set set_connection_lines_curvature; get get_connection_lines_curvature]
  The curvature of the lines between the nodes. 0 results in straight lines.

- connection_lines_thickness: float = 4.0 [set set_connection_lines_thickness; get get_connection_lines_thickness]
  The thickness of the lines between the nodes.

- connections: Dictionary[] = [] [set set_connections; get get_connection_list]
...
- grid_pattern: int (GraphEdit.GridPattern) = 0 [set set_grid_pattern; get get_grid_pattern]
  The pattern used for drawing the grid.

- minimap_enabled: bool = true [set set_minimap_enabled; get is_minimap_enabled]
  If true, the minimap is visible.

- minimap_opacity: float = 0.65 [set set_minimap_opacity; get get_minimap_opacity]
  The opacity of the minimap rectangle.

- minimap_size: Vector2 = Vector2(240, 160) [set set_minimap_size; get get_minimap_size]
  The size of the minimap rectangle. The map itself is based on the size of the grid area and is scaled to fit this rectangle.

- panning_scheme: int (GraphEdit.PanningScheme) = 0 [set set_panning_scheme; get get_panning_scheme]
  Defines the control scheme for panning with mouse wheel.

- right_disconnects: bool = false [set set_right_disconnects; get is_right_disconnects_enabled]
  If true, enables disconnection of existing connections in the GraphEdit by dragging the right end.

- scroll_offset: Vector2 = Vector2(0, 0) [set set_scroll_offset; get get_scroll_offset]
  The scroll offset.

- show_arrange_button: bool = true [set set_show_arrange_button; get is_showing_arrange_button]
  If true, the button to automatically arrange graph nodes is visible.

- show_grid: bool = true [set set_show_grid; get is_showing_grid]
  If true, the grid is visible.

- show_grid_buttons: bool = true [set set_show_grid_buttons; get is_showing_grid_buttons]
  If true, buttons that allow to configure grid and snapping options are visible.

- show_menu: bool = true [set set_show_menu; get is_showing_menu]
  If true, the menu toolbar is visible.

- show_minimap_button: bool = true [set set_show_minimap_button; get is_showing_minimap_button]
  If true, the button to toggle the minimap is visible.

- show_zoom_buttons: bool = true [set set_show_zoom_buttons; get is_showing_zoom_buttons]
  If true, buttons that allow to change and reset the zoom level are visible.

- show_zoom_label: bool = false [set set_show_zoom_label; get is_showing_zoom_label]
  If true, the label with the current zoom level is visible. The zoom level is displayed in percents.

- snapping_distance: int = 20 [set set_snapping_distance; get get_snapping_distance]
  The snapping distance in pixels, also determines the grid line distance.

- snapping_enabled: bool = true [set set_snapping_enabled; get is_snapping_enabled]
  If true, enables snapping.

- type_names: Dictionary = {} [set set_type_names; get get_type_names]
  Dictionary of human-readable port type names.

- zoom: float = 1.0 [set set_zoom; get get_zoom]
  The current zoom value.

- zoom_max: float = 2.0736003 [set set_zoom_max; get get_zoom_max]
  The upper zoom limit.

- zoom_min: float = 0.23256795 [set set_zoom_min; get get_zoom_min]
  The lower zoom limit.

- zoom_step: float = 1.2 [set set_zoom_step; get get_zoom_step]
