...
- Source: EditorContextMenuPlugin.xml
- Inherits: RefCounted
- Inheritance Chain: EditorContextMenuPlugin -> RefCounted -> Object
...
```
[methods]
_popup_menu(paths: PackedStringArray) -> void [virtual]
add_context_menu_item(name: String, callback: Callable, icon: Texture2D = null) -> void
add_context_menu_item_from_shortcut(name: String, shortcut: Shortcut, icon: Texture2D = null) -> void
add_context_submenu_item(name: String, menu: PopupMenu, icon: Texture2D = null) -> void
...
```
func _popup_menu(paths):
    var code_edit = Engine.get_main_loop().root.get_node(paths0);
```
...
```
func _popup_menu(paths):
    var canvas_item = Engine.get_main_loop().root.get_node(paths0); # Replace 0 with the desired index.
```
