...
- Name: Geometry2D
- Source: Geometry2D.xml
- Inherits: Object
- Inheritance Chain: Geometry2D -> Object
...
offset_polygon(polygon: PackedVector2Array, delta: float, join_type: int (Geometry2D.PolyJoinType) = 0) -> PackedVector2Array[]
offset_polyline(polyline: PackedVector2Array, delta: float, join_type: int (Geometry2D.PolyJoinType) = 0, end_type: int (Geometry2D.PolyEndType) = 3) -> PackedVector2Array[]
point_is_inside_triangle(point: Vector2, a: Vector2, b: Vector2, c: Vector2) -> bool [const]
...
- exclude_polygons(polygon_a: PackedVector2Array, polygon_b: PackedVector2Array) -> PackedVector2Array[]
  Mutually excludes common area defined by intersection of polygon_a and polygon_b (see intersect_polygons()) and returns an array of excluded polygons. This performs OPERATION_XOR between polygons. In other words, returns all but common area between polygons. The operation may result in an outer polygon (boundary) and inner polygon (hole) produced which could be distinguished by calling is_polygon_clockwise().
...
- is_polygon_clockwise(polygon: PackedVector2Array) -> bool
  Returns true if polygon's vertices are ordered in clockwise order, otherwise returns false. **Note:** Assumes a Cartesian coordinate system where +x is right and +y is up. If using screen coordinates (+y is down), the result will need to be flipped (i.e. a true result will indicate counter-clockwise).

- line_intersects_line(from_a: Vector2, dir_a: Vector2, from_b: Vector2, dir_b: Vector2) -> Variant
  Returns the point of intersection between the two lines (from_a, dir_a) and (from_b, dir_b). Returns a Vector2, or null if the lines are parallel. from and dir are *not* endpoints of a line segment or ray but the slope (dir) and a known point (from) on that line.
