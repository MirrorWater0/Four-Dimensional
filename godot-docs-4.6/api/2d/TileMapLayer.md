...
- Name: TileMapLayer
- Source: TileMapLayer.xml
- Inherits: Node2D
- Inheritance Chain: TileMapLayer -> Node2D -> CanvasItem -> Node -> Object
...
```
[methods]
_tile_data_runtime_update(coords: Vector2i, tile_data: TileData) -> void [virtual]
_update_cells(coords: Vector2i[], forced_cleanup: bool) -> void [virtual]
_use_tile_data_runtime_update(coords: Vector2i) -> bool [virtual]
clear() -> void
...
get_cell_tile_data(coords: Vector2i) -> TileData [const]
get_coords_for_body_rid(body: RID) -> Vector2i [const]
...
get_used_cells_by_id(source_id: int = -1, atlas_coords: Vector2i = Vector2i(-1, -1), alternative_tile: int = -1) -> Vector2i[] [const]
get_used_rect() -> Rect2i [const]
has_body_rid(body: RID) -> bool [const]
is_cell_flipped_h(coords: Vector2i) -> bool [const]
is_cell_flipped_v(coords: Vector2i) -> bool [const]
is_cell_transposed(coords: Vector2i) -> bool [const]
...
set_cell(coords: Vector2i, source_id: int = -1, atlas_coords: Vector2i = Vector2i(-1, -1), alternative_tile: int = 0) -> void
set_cells_terrain_connect(cells: Vector2i[], terrain_set: int, terrain: int, ignore_empty_terrains: bool = true) -> void
set_cells_terrain_path(path: Vector2i[], terrain_set: int, terrain: int, ignore_empty_terrains: bool = true) -> void
...
- _use_tile_data_runtime_update(coords: Vector2i) -> bool [virtual]
  Should return true if the tile at coordinates coords requires a runtime update. **Warning:** Make sure this function only returns true when needed. Any tile processed at runtime without a need for it will imply a significant performance penalty. **Note:** If the result of this function should change, use notify_runtime_tile_data_update() to notify the TileMapLayer it needs an update.
...
- get_coords_for_body_rid(body: RID) -> Vector2i [const]
  Returns the coordinates of the physics quadrant (see physics_quadrant_size) for given physics body RID. Such an RID can be retrieved from KinematicCollision2D.get_collider_rid(), when colliding with a tile. **Note:** Higher values of physics_quadrant_size will make this function less precise. To get the exact cell coordinates, you need to set physics_quadrant_size to 1, which disables physics chunking.

- get_navigation_map() -> RID [const]
  Returns the RID of the NavigationServer2D navigation used by this TileMapLayer. By default this returns the default World2D navigation map, unless a custom map was provided using set_navigation_map().
...
- collision_enabled: bool = true [set set_collision_enabled; get is_collision_enabled]
  Enable or disable collisions.

- collision_visibility_mode: int (TileMapLayer.DebugVisibilityMode) = 0 [set set_collision_visibility_mode; get get_collision_visibility_mode]
...
- navigation_enabled: bool = true [set set_navigation_enabled; get is_navigation_enabled]
  If true, navigation regions are enabled.

- navigation_visibility_mode: int (TileMapLayer.DebugVisibilityMode) = 0 [set set_navigation_visibility_mode; get get_navigation_visibility_mode]
...
- occlusion_enabled: bool = true [set set_occlusion_enabled; get is_occlusion_enabled]
  Enable or disable light occlusion.

- physics_quadrant_size: int = 16 [set set_physics_quadrant_size; get get_physics_quadrant_size]
...
- tile_map_data: PackedByteArray = PackedByteArray() [set set_tile_map_data_from_array; get get_tile_map_data_as_array]
  The raw tile map data as a byte array.

- tile_set: TileSet [set set_tile_set; get get_tile_set]
  The TileSet used by this layer. The textures, collisions, and additional behavior of all available tiles are stored here.

- use_kinematic_bodies: bool = false [set set_use_kinematic_bodies; get is_using_kinematic_bodies]
...
- x_draw_order_reversed: bool = false [set set_x_draw_order_reversed; get is_x_draw_order_reversed]
