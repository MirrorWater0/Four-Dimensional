...
public partial class Battle
{
    ...
    private void OnSkillUsed(Skill skill)
    {
        ...
        TriggerSkillUsedRelicEffects(skill);
    }

    private async void TriggerSkillUsedRelicEffects(Skill skill)
    {
        try
        {
            ...
            if (skill == null || relics == null || !CanContinue(_lifetimeCts.Token))
                return;

            foreach (var relic in new List<Relic>(relics))
            {
                if (relic == null || !CanContinue(_lifetimeCts.Token))
                    return;

                await relic.OnSkillUsed(this, skill);
            }
        }
        catch (Exception ex)
        {
            GD.PrintErr($"[Relic] skill-used effect failed: {ex}");
        }
    }
