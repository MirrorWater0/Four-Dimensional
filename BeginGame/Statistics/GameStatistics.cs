...
public partial class GameStatistics : CanvasLayer
{
    ...
    private void RefreshSummary()
    {
        ...
        AddSummaryChip($"装备 {CountEquipmentRecords(GetEquipmentRecords())}", Colors.White, new Color(0.23f, 0.24f, 0.34f, 0.92f));
        AddSummaryChip($"遗物 {CountRelicRecords(_currentRecord.RelicRecords)}", Colors.White, new Color(0.32f, 0.24f, 0.12f, 0.92f));
    }
    ...
    private static int CountEquipmentRecords(IEnumerable<RunHistoryEquipmentRecord> records)
    {
        return records
                ?.Where(record => record != null)
                .Sum(record => Math.Max(1, record.Count))
            ?? 0;
    }

    private static int CountRelicRecords(IEnumerable<RunHistoryRelicRecord> records)
    {
        return records
                ?.Where(record => record != null)
                .Sum(record => Math.Max(1, record.Count))
